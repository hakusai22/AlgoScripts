...
// 用闭包实现累加器
// 解释：1.AddUpper是一个函数，返回的数据类型是匿名函数  05_func(int) int 即闭包
//
//	2.返回的匿名函数 05_func(x int) int 和 其引用的函数外的变量n 构成闭包
...
// 功能要求：
// 1)编写一个函数 makeSuffix(suffix 07_string)  可以接收一个文件后缀名(比如.jpg)，并返回一个闭包
...
// 使用闭包实现说明：
// 1.返回的匿名函数func (name 07_string) 07_string 和 输入变量suffix构成闭包
// 2.优势是不用每次调用都传入 ".jpg"
