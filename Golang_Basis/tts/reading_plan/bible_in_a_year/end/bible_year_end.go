...
func main() {
	//prayer := ""
	file, err := xlsx.OpenFile("./Golang_Basis/tts/bible_year_end.xlsx")
	if err != nil {
		log.Fatal(err)
	}
	...
	for rowIndex, row := range sheet.Rows {
		...
		prefix := "bible_in_a_year_"
		introduction_key = strings.TrimPrefix(introduction_key, prefix)
...
func handleTTS3(introduction_key, introduction_content string) string {
	...
	keyName := fmt.Sprintf("wepray_business/reading_plan/bible_year/%s.mp3", introduction_key)
	...
	file_txt, err := os.OpenFile("./end_urls.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	// 创建一个新的写入器
