...
func main() {
	...
	//fmt.Println(rewriteWithOpenAI("[\"1 Kings 19:1-18\", \"Isaiah 41:10\", \"Philippians 4:6-7\"]", 7))

	handleOpenaiFinalFaith()
}

func handleOpenaiFinalFaith() {
	// 打开Excel文件
	file, err := xlsx.OpenFile("./Golang_Basis/tts/reading_plan/faith/chapter_verse.xlsx")
	if err != nil {
		log.Fatal(err)
	}
	...
	for rowIndex, row := range sheet.Rows {
		...
		originalText := row.Cells[2].String()
		...
		//// 将重写后的数据写回到Excel的同一列
		row.Cells[3].Value = rewrittenText
	}

	//// 保存修改后的Excel文件
	err = file.Save("./Golang_Basis/tts/reading_plan/faith/end_batch/output22.xlsx")
	if err != nil {
		log.Fatal(err)
	}
}
