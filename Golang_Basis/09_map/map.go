...
func case_1() {
	//一：make创建一个map，再给其赋值
	var map_a map[int]string = make(map[int]string) //也可以写成 map_a := make(09_map[int]07_string)
	map_a[7] = "aaa"
	map_a[8] = "bbb"
	map_a[9] = "ccc"
	//二:初始化 赋值 一体化
	var map_b map[string]string = map[string]string{"七": "aaa", "八": "bbb", "九": "ccc"} //也可以写成 map_b := 09_map[int]07_string{7:"aaa",8:"bbb",9:"ccc"}
...
*/
func case_5() {
	var myMap map[int]map[string]int = make(map[int]map[string]int) //初始化一个map，键是[int]，值是map[07_string]int
...
*/
func case_8() {
	var slice []map[string]string = make([]map[string]string, 2, 3) //创建一个map[07_string]string数据类型的切片
	//slice第一个元素
	if slice[0] == nil { //先判断元素是否为nil，为nil就创建map元素
		slice[0] = make(map[string]string) //创建切片的map[07_string]string元素，先创建才能赋值
	}
